...
export const LIBRARY_COMPONENTS_MAP: Record<
  string,
  Record<string, { name: string; description: string }[]>
> = {};

Object.entries(
  import.meta.glob<any[]>("../../library/components/**/*/dump.json", {
    import: "default",
    eager: true,
  })
).forEach(([k, v]) => {
...
//   `utf-8`,
// ),
export const IMPORTS_LISTS: Record<
  string,
  Record<string, { import: string[] }>
> = {};

Object.entries(
  import.meta.glob<any>("../../library/components/**/*/metadata.json", {
    import: "default",
    eager: true,
  })
).forEach(([k, v]) => {
...
export function createComponentsSchema(options: any) {
  return z.object({
    ...
    new_component_icons_elements: z.object({
      ...
      if_so_what_new_component_icons_elements_are_needed: z.array(
        z.string().optional()
      ),
    }),
    use_library_components: z.array(
      z.object({
        library_component_name: z
          .enum(
            LIBRARY_COMPONENTS_MAP[options.query.framework][
              options.query.components
            ].map((e) => e.name) as [string, ...string[]]
          )
          .optional(),
